...
data<-read.csv("Adaptation with gravity vars amended FULL Feb 14 2023.csv")
...
regDataAdapt<-regData1[complete.cases(regData1),]

names(regDataAdapt)[5]<-"AdaptAmount"
names(regDataAdapt)[6]<-"MitiAmount"

## remove the zero provider countries
regDataAdapt$ProviderISO<-as.character(regDataAdapt$ProviderISO)
regDataAdapt$RecipientISO<-as.character(regDataAdapt$RecipientISO)

library(fastDummies)

regDataAdaptD<-dummy_cols(regDataAdapt, select_columns = c("Sector","RecipientISO","ProviderISO","Year"), remove_first_dummy = FALSE)

## column 42 to 110 are recipient
## 69 countries 
names(regDataAdaptD)[44:111]

reg1<-regDataAdaptD[,c(5:149)]
